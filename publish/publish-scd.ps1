...
$rIdLine =  Get-Content "$PSScriptRoot\..\src\hello-netcoreapp.csproj" | Select-String -Pattern "<RuntimeIdentifiers>.*</RuntimeIdentifiers>"
...
foreach ($rId in $rIds) {
    ...
    dotnet restore $PSScriptRoot\..\src -r $rId
    ...
    dotnet publish $PSScriptRoot\..\src -r $rId -c release -o "$publishDir"
